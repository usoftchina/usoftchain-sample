...
instantiateChaincode () {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		peer chaincode instantiate -o orderer.example.com:7050 -C $CHANNEL_NAME -n stockcontract -v 1.0 -c '{"Args":["init"]}' -P "AND ('huaslMSP.peer','skypineMSP.peer','xinningMSP.peer','usoftMSP.peer')" >&log.txt
	else
		peer chaincode instantiate -o orderer.example.com:7050 --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n stockcontract -v 1.0 -c '{"Args":["init"]}' -P "AND ('huaslMSP.peer','skypineMSP.peer','xinningMSP.peer','usoftMSP.peer')" >&log.txt
	fi
	res=$?
	cat log.txt
